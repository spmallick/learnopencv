...
# Function to modify the rotation matrix to represent y-axis rotation
...
def train():
    ...
    if args.dataset_type == 'llff':
        images, poses, bds, render_poses, render_poses_xz, i_test = load_llff_data(args.datadir, args.factor,
    ...
    if render_poses_xz is not None:
        render_poses_xz = torch.Tensor(render_poses_xz).to(device)
    else:
        print("render_poses_xz is None, skipping its conversion to Tensor.")
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            # cv2.imwrite(os.path.join(infer_path, f"infer_rgb.png"), cv2.cvtColor(to8b(rgbs[0]), cv2.COLOR_RGB2BGR))
            # cv2.imwrite(os.path.join(infer_path, f"infer_disp.png"), to8b(disps[0] / np.max(disps[0])))
    ...
    if use_batching:
        ...
        print('get rays')
        rays = np.stack([get_rays_np(H, W, K, p) for p in poses[:,:3,:4]], 0) # [N, ro+rd, H, W, 3] -> (335, 2, 640, 360, 3)
        print("=------------------------------")
        print(poses[:,:3,:4].shape)
        print('done, concats')
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if N_rand is not None:
                rays_o, rays_d = get_rays(H, W, K, torch.Tensor(pose))  # (H, W, 3), (H, W, 3)  # noqa: F405
        ...
        if i%1000==0 and i >= 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(render_poses[0,:,:].unsqueeze(0), hwf, K, args.chunk, render_kwargs_test)
