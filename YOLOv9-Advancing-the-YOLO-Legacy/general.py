...
def one_flat_cycle(y1=0.0, y2=1.0, steps=100):
    ...
    # return lambda x: ((1 - math.cos(x * math.pi / steps)) / 2) * (y2 - y1) + y1
    return lambda x: (
        ((1 - math.cos((x - (steps // 2)) * math.pi / (steps // 2))) / 2) * (y2 - y1) + y1 if (x > (steps // 2)) else y1
    )
...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # YOLO model in validation model, output = (inference_out, loss_out)
        prediction = prediction[0][
            1
        ]  # select only inference output, [0][0] for aux prediction, [0][1] for main prediction, [0] for  re-parameterized prediction.
