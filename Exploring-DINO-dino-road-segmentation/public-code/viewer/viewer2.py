...
def view_image_label_pair(image_path, label_path):

    # image_path = label_path.replace('gtFine', 'leftImg8bit').replace('_labelids','')
    label_mask = imread(label_path)
    image = imread(image_path)
    imgs = [image]
    imgs.append(get_image(label_mask,2))


    f = plt.figure(figsize=(14,9), dpi=150)
            
    axarr = [None]*5
    axarr[0] = f.add_subplot(121)
    axarr[1] = f.add_subplot(122)
...
def get_args():
    ...
    parser.add_argument('--masks', default="/ssd_scratch/cvit/girish.varma/dataset/anue_test/gtFine/test")
    parser.add_argument('--imgs', default="/ssd_scratch/cvit/girish.varma/dataset/anue_test/leftImg8bit/test")
...
# The main method
def main(args):
    image_path_list = glob.glob(args.imgs+'/*/*_leftImg8bit.png')
    random.shuffle(image_path_list)
    # print(image_path_list[0].replace('_leftImg8bit.png', '_gtFine_labellevel3Ids.png').replace(args.imgs, args.masks))
    mask_path_list = [ i.replace('_leftImg8bit.png', '_gtFine_labellevel3Ids.png').replace(args.imgs, args.masks) for i in image_path_list ]

    for l in range(len(image_path_list)):
        print(image_path_list[l], mask_path_list[l])
        view_image_label_pair(image_path_list[l], mask_path_list[l])
