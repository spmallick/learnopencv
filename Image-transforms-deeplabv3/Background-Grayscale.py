...
def decode_segmap(image, source, nc=21):
  ...
  source = source.resize((r.shape[1],r.shape[0])).convert('RGB')
  ...
  for x in range(width):
      for y in range(height):
          ...
          if r > 0 and g > 0 and b > 0:
              ...
              # If its background convert the RGB pixels at the corresponding x,y position to grayscale 
              # before adding them to output image
              rb, gb, bb = pixels[x,y]
              grey = int((rb + gb + bb)/3)              
              out.putpixel((x,y), (grey, grey, grey))
...
segment(dlab, './images/greyscale/boat.png', show_orig=False)
segment(dlab, './images/greyscale/oldman.png', show_orig=False)
