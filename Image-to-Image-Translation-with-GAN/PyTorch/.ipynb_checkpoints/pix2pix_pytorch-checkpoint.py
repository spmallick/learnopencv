...
def imshow(inputs, target, figsize=(15, 15)):
...
generator = UnetGenerator(3, 3, 8, 64, norm_layer=norm_layer, use_dropout=False).cuda().float()
init_weights(generator, 'normal', init_gain=0.02)
...
discriminator = NLayerDiscriminator(6, 64, n_layers=3, norm_layer=norm_layer).cuda().float()
...
def generator_loss(generated_image, target_img, G, real_target):
    ...
    #print(gen_loss)
    return gen_loss, l1_l, gen_total_loss
...
#         print('fake_gen:', fake_gen)
        ...
        gen_loss, l1_l, gen_total_loss = generator_loss(generated_image, target_img, G, real_target)                                 
    ...
    print('Epoch: [%d/%d]: D_loss: %.3f, G_loss: %.3f, L1_loss: %.3f'  % (
            (epoch), num_epochs, torch.mean(torch.FloatTensor(D_loss_list)),\
             torch.mean(torch.FloatTensor(G_loss_list)),  torch.mean(torch.FloatTensor(L1_loss_list))))
